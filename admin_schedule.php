...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['cancel_schedule_id'])) {
        ...
        if ($count > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO schedules (user_id, date, start_time, end_time, aircraft_id) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("isssi", $user_id, $date, $start_time, $end_time, $aircraft_id);
            $stmt->execute();
            $stmt->close();
        }
    } else {
...
<body>
    ...
    <section class="schedule-container">
        ...
        <table class="schedule-table">
            ...
            <tbody>
                <?php
                $schedules = $conn->query("SELECT schedules.*, users.nome_de_pista, aeronaves.matricula, aeronaves.modelo FROM schedules JOIN users ON schedules.user_id = users.id JOIN aeronaves ON schedules.aircraft_id = aeronaves.id ORDER BY date, start_time");
